...
static void renderOnce(DualChain& dual, double sr, int block, int chans,
...
int main()
{
    ...
    auto mono = makeDeterministicInput<float>(total, 0xD17E5Tu);
    std::vector<float> inL, inR; monoToStereo(mono, inL, inR);

    DualChain dual;

    FieldChain::Config cfg{};
    cfg.enableDelay = cfg.enableDynEq = cfg.enableReverb = false;
    cfg.enableMeter = cfg.enableMS = cfg.enableGain = false;
    ...
    {
        dual.promoteStagingHard(); // ensure both sides are identical, then arm swap
        (void) dual.armLiveSwapIfSameLatency();
        renderOnce(dual, sr, b, chans, inL, inR, outL, outR);

        assert(outL.size()==inL.size() && outR.size()==inR.size());
